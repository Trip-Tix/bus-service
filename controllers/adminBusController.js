    ...
    jwt.verify(token, secretKey, async (err, decoded) => {
        if (err) {
            ...
            try {
                ...
                const {busName, availableNumber, adminUsernameArray, coachName, availableSeat, layout, row, column} = req.body;
                const numberOfBuses = availableNumber;  
                const numberOfSeats = availableSeat;
                const matrix = layout;
                ...
                if (checkResult.rows.length > 0) {
                    ...
                    // Update bus coach info
                    // Get the bus id
                    const busIdQuery = {
                        text: 'SELECT bus_id, coach_info FROM bus_services WHERE bus_name = $1',
                        values: [busName]
                    };
                    ...
                    // Check if coach name already exists in coach info
                    if (coachInfo.includes(coachName)) {
                        console.log("Coach name already exists");
                        return res.status(400).json({ message: 'Coach name already exists' });
                    }
                    coachInfo.push(coachName);
                    console.log(coachInfo);

                    // Update the bus services table with new total number of buses and coach info
                    const updateQuery = {
                        text: 'UPDATE bus_services SET coach_info = $1 WHERE bus_id = $2',
                        values: [coachInfo, busId]
                    };
                    await busPool.query(updateQuery);
                    ...
                    const coachIdResult = await busPool.query(coachIdQuery);
                    if (coachIdResult.rows.length === 0) {
                        console.log("Coach does not exist");
                        
                        // Insert the coach name into coach_info table
                        const insertCoachQuery = {
                            text: 'INSERT INTO coach_info (coach_name) VALUES ($1)',
                            values: [coachName]
                        };
                        await busPool.query(insertCoachQuery);
                        console.log("Coach added");
                    }

                    // Insert the bus coach info into bus_coach_info table
                    const busCoachId = busName + "_" + coachName + "_" + coachInfo.length;
                    const busCoachQuery = {
                        text: 'INSERT INTO bus_coach_info (bus_coach_id, bus_id, coach_name, number_of_buses) VALUES ($1, $2, $3, $4)',
                        values: [busCoachId, busId, coachName, numberOfBuses]
                    };
                    ...
                    // Add bus layout info
                    const layoutQuery = {
                        text: 'INSERT INTO bus_layout_info (bus_id, coach_name, number_of_seats, matrix_rows, matrix_cols, bus_coach_id) VALUES ($1, $2, $3, $4, $5, $6)',
                        values: [busId, coachName, numberOfSeats, matrixRows, matrixCols, busCoachId]
                    };
                    await busPool.query(layoutQuery);
                    console.log("Bus layout added");
                    
                    // Add bus seat details
                    // Get the bus layout id
                    const layoutIdQuery = {
                        text: 'SELECT bus_layout_id FROM bus_layout_info WHERE bus_coach_id = $1',
                        values: [busCoachId]
                    };
                    const layoutIdResult = await busPool.query(layoutIdQuery);
                    const layoutId = layoutIdResult.rows[0].bus_layout_id;
                    console.log("Bus layout id", layoutId);
                    // Add layout details to bus_seat_details table
                    for(let i = 0; i < matrixRows; i++) {
                        let colCount = 0;
                        for(let j = 0; j < matrixCols; j++) {
                            const seatName = String.fromCharCode(65 + i) + (colCount + 1);
                            const isSeat = matrix[i][j];
                            const seatQuery = {
                                text: 'INSERT INTO bus_seat_details (bus_layout_id, seat_name, is_seat, matrix_row_id, matrix_col_id) VALUES ($1, $2, $3, $4, $5)',
                                values: [layoutId, seatName, isSeat, i, j]
                            };
                            await busPool.query(seatQuery);
                        }
                    }
                    console.log("Bus seat details added");
                    res.status(200).json({ message: 'Bus information updated', bus_id: busId });
                } else {
                    // Get admin id from admin username array
                    const adminIdQuery = {
                        text: 'SELECT admin_id FROM admin_info WHERE username = ANY($1::text[])',
                        values: [adminUsernameArray]
                    };
                    const adminIdResult = await accountPool.query(adminIdQuery);
                    console.log(adminIdResult.rows);
                    if (adminIdResult.rows.length !== adminUsernameArray.length) {
                        console.log("Admin does not exist");
                        return res.status(400).json({ message: 'Admin does not exist' });
                    }
                    // Get the admin id array in numbers from adminIdResult.rows
                    const adminIdArray = adminIdResult.rows.map(admin => admin.admin_id);
                    console.log(adminIdArray);
                    let coachInfo = [];
                    coachInfo.push(coachName);
                    const query = {
                        text: 'INSERT INTO bus_services (bus_name, admin_id, coach_info) VALUES ($1, $2, $3)',
                        values: [busName, adminIdArray, coachInfo]
                    };
                    ...
                    console.log("Bus seat details added");
                    res.status(200).json({ message: 'Bus added', bus_id: busId });
                }
            } catch (error) {
